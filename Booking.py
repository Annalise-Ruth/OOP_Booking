...
DISTANCE_MATRIX = {
    ...
    ("CEA", "PUP LHS"): 2.0, ("CEA", "Condotel"): 2.5,
    ("Hasmin", "iTech"): 0.5, ("Hasmin", "COC"): 1.0, ("Hasmin", "PUP LHS"): 1.5,
    ("Hasmin", "Condotel"): 2.0,
...
def get_distance(start, end):
    if start == end:
        return 0
    return DISTANCE_MATRIX.get((start, end)) or DISTANCE_MATRIX.get((end, start)) or 0
    ...
    def load(self):
        try:
            with open(self.file, "r") as f:
                data = json.load(f)
                for d in data:
                    # Safe loading: skip if missing vehicle_type
